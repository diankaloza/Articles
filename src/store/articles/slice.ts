...
import { I_ArticlesSlice, T_VisibleArticle } from './types'

import { T_Article } from 'models/article'
import { E_Status } from 'models/store'

export const fetchArticles = createAsyncThunk<T_Article[]>('articles/fetchByIdStatus', async () => {
...
export const articlesSlice = createSlice({
  ...
  initialState,
  reducers: {
    setVisibleArticlesBySearchInput: (state, action: PayloadAction<{ searchValue: string }>) => {
        ...
        state.visibleArticles = state.articles
          .reduce((acc, current) => {
            ...
            keywords.forEach((keyword: string) => {
              ...
              if (current.description.toLowerCase().substring(0, 100).includes(keyword)) {
